...
export async function GET(request: Request, context: RouteContext) {
  try {
    ...
    const texts = await listAgentTexts(agentId);
    ...
    console.error("Failed to load agent texts from Postgres:", error);
    return NextResponse.json({ error: "Failed to load agent texts." }, { status: 500 });
  }
}
