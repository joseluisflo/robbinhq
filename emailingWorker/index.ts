...
export default {
  ...
  async email(message, env, ctx) {
    ...
    try {
      ...
      // Usamos la nueva función robusta para obtener el cuerpo del email.
      const body = await getBody(rawEmail);
...
// --------------------------------------------------
// --- LÓGICA MEJORADA PARA PARSEAR EL CUERPO DEL EMAIL ---
// --------------------------------------------------

async function getBody(rawEmail) {
  ...
  if (!contentTypeHeader) return bodyPart.trim();

  const contentType = contentTypeHeader[0];

  if (contentType.includes('multipart/')) {
    const boundaryMatch = contentType.match(/boundary="?([^"]+)"?/i);
    if (!boundaryMatch) return bodyPart.trim();
  ...
  if (headersPart.match(/^Content-Transfer-Encoding: quoted-printable/im)) {
      return decodeQuotedPrintable(bodyPart);
  }
